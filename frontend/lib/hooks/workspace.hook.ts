...
// get pending invitations sent for a specific workspace
export function useGetWorkspacePendingInvitations(workspaceId: string) {
  return useQuery({
    queryKey: ["workspace-pending-invitations", workspaceId],
    queryFn: () => workspaceApi.getWorkspacePendingInvitations(workspaceId),
    enabled: !!workspaceId,
  });
}
...
export const useCancelWorkspaceInvitation = () => {
  ...
  return useMutation({
    mutationFn: (inviteId: string) =>
      workspaceApi.cancelWorkspaceInvitation(inviteId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["workspace-pending-invitations"] });
...
export const useAcceptWorkspaceInvitation = () => {
  ...
  return useMutation({
    mutationFn: (inviteId: string) =>
      workspaceApi.acceptWorkspaceInvites(inviteId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["workspaces"] });
...
export const useRejectWorkspaceInvitation = () => {
  ...
  return useMutation({
    mutationFn: (inviteId: string) =>
      workspaceApi.rejectWorkspaceInvites(inviteId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["workspaces"] });
      queryClient.invalidateQueries({ queryKey: ["feeds"] });
      queryClient.invalidateQueries({ queryKey: ["workspace-invitations"] });
